...
pred_mod <- readRDS(paste0(proj_box_path, "Github_LFS/Fitted_Mods/PredFit_SpST.rds"))
lob_mod <- readRDS(paste0(proj_box_path, "Github_LFS/Fitted_Mods/JuveFit_SpST.rds"))
adult_mod <- readRDS(paste0(proj_box_path, "Github_LFS/Fitted_Mods/AdultFit_SpST.rds"))
...
mm_season <- model.matrix(~ 0 + season, data = prediction_grid)
...
juve_yhat <- predict(lob_mod, newdata = prediction_grid, type = "response")
pred_yhat <- predict(pred_mod, newdata = prediction_grid, type = "response")
...
overlaps_1deg <- df_overlap3 %>% 
  ...
  mutate(p_res = predator_est, # Here I just use the predicted estimates on the response scale. However, should we use get_index() to estimate relative biomas in each spatial unit???
         l_res = lobster_est) # %>%
