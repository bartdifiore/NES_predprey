...
lobster <- readRDS(here::here("Data/Derived/all_model_data_juvenile.rds")) %>%
  ...
  filter(lobster_biomass <= 2000) # filter out the large tow which was > 2000 kg.
...
# sanity(fit_lp2)
# 
# write_rds(fit_lp2, "Data/Derived/Pred_Fit_lp2.rds", compress = "gz")
...
tictoc::tic()
fit_lp3 <- sdmTMB(
  lobster_biomass ~ factor(season) + factor(survey) + s(Depth_scaled, k = 4) + s(BT_seasonal_scaled, k = 4),
  # control = sdmTMBcontrol(map = list(ln_tau_Z = tau_Z_map)),
  data = mod_data3,
  spatial = "on",
  # offset = dat_mod$swept,
...
write_rds(fit_lp3, paste0(proj_box_path, "Github_LFS/Fitted_Mods/hybrid3.rds"), compress = "gz")
#---------------------------------
## Predict
#---------------------------------
