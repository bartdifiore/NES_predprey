...
ggplot(preds, aes(x = forcats::fct_reorder(com_name, frequency, .desc = T), y = frequency))+
  geom_bar(stat = "identity")+
  geom_text(aes(label = nstom), nudge_y = 1.25, size = 3)+
  labs(y = "Total number of stomachs lobster were observed in", x = "")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust=1))

ggsave("Figures/lobster_predators.png")
